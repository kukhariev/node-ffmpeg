...
const ffmpegPath = (process.env.FFMPEG_PATH = String(process.env.FFMPEG_PATH || ffmpegStaticPath));

function generateTestFile(filename, durationSec = 10.0) {
  spawnSync(ffmpegPath, ['-y', '-f', 'lavfi', '-i', `testsrc=duration=${durationSec}:size=640x360:rate=30`, filename]);
}
